...
def headers(version):
    if version == 1.1:
        return {'Content-Type': 'text/xml; charset=utf-8',
                'SOAPAction': 'http://microsoft.com/webservices/DatosLey103'}
    elif version == 1.2:
...
def data(version, year, month=None):
    ...
    month = '<mes>{}</mes>'.format(month) if month else ''
    if version == 1.1:
...
r = requests.post(url, headers=headers(1.1), data=data(1.1, 2010))